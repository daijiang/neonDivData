#' Breeding landbird point count data
#'
...
#'
#' 1. Filtered `brd_perpoint` to `samplingImpractical == "OK"` to retain only surveyed points.
#' 2. Left-joined downward from point survey records to `brd_countdata` to preserve zero-bird survey points.
#' 3. Retained valid cluster size values and zero-bird survey rows; `value` = cluster size (count of individuals).
#' 4. `unique_sample_id` = `eventID`.
#'
