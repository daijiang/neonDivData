...
NEON releases new data each January. The package is updated annually,
typically in March, by running the pipeline in `data-raw/` locally and
pushing the rebuilt package data to GitHub.

**Prerequisites:** a NEON API token (register at
[data.neonscience.org](https://data.neonscience.org)) stored in your
`.Renviron` as `NEON_TOKEN=your_token_here`.
...
`make all` runs three scripts in sequence, skipping any step whose
...
If the download is interrupted, re-run `make all` and it will resume
from step 2 if the RDS files are present, or repeat only the failed
taxon downloads if you re-run step 1 manually. To force a specific step
without re-running earlier ones, use `make download`, `make clean-data`,
or `make plants`.
