...
# set to TRUE each year after a new NEON release to re-download everything
...
neon_ecocomdp_data_list <- filter(neon_ecocomdp_data_list, !id %in% c(
  "neon.ecocomdp.10022.001.002",
  "neon.ecocomdp.20107.001.001"
))
#  https://github.com/EDIorg/ecocomDP/issues/157
#  https://github.com/EDIorg/ecocomDP/issues/156
...
for (i in 1:nrow(neon_ecocomdp_data_list)) {
  ...
  # download data
  try({
    file_downloded <- !force_redownload && any(grepl(pattern = my_dp_id, x = list.files(my_out_dir)))
    if (file_downloded) { # already downloaded, just read in
      data_list_i <- readRDS(grep(
        pattern = my_dp_id,
      ...
      id_f <- sort(grep(str_replace(str_replace(my_dp_id, "neon\\.ecocomdp", "DP1"), "\\.[0-9]{3}$", ""),
        list.files(my_raw_dir, full.names = T),
        value = T
      ))
      if (length(id_f) > 0) {
