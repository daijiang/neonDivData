  ...
  try({
    if(length(data_list_i) > 0 && nrow(data_flat_i) > 0){
      ...
      data_set_log_i <- data.frame(
        ...
        start_date = data_flat_i$observation_datetime %>% lubridate::as_date() %>% min(na.rm = TRUE),
        end_date = data_flat_i$observation_datetime %>% lubridate::as_date() %>% max(na.rm = TRUE),
