...
map_neon_data_to_ecocomDP.FISH <- function(
  ...
  all_fish <- neonUtilities::loadByProduct(dpID = neon.data.product.id,
                                           package = "expanded", ...)
  # saveRDS(all_fish, file = "~/Documents/allTabs_fish.rds")
  # all_fish = readRDS("~/Documents/allTabs_fish.rds")
  ...
  fsh_dat1 <- dplyr::left_join(x = all_fish$fsh_perPass, y = all_fish$fsh_fieldData,
                               by = c('reachID')) %>%
  ...
  my_grouping_vars <- c('domainID','siteID','aquaticSiteType','namedLocation',
                        ...
                        'efTime2', "passStartTime", "passEndTime", 'netDeploymentTime', 'taxonID', 'scientificName', 'passNumber')
  ...
  fsh_dat_aggregate$aquaticSiteType <-
    dplyr::if_else(is.na(fsh_dat_aggregate$aquaticSiteType),
                   dplyr::if_else(grepl("fish.point", fsh_dat_aggregate$namedLocation),
                                  'stream','lake'),
                   fsh_dat_aggregate$aquaticSiteType)
  ...
  data_fish = left_join(data_fish, unique(dplyr::select(all_fish$fsh_fieldData, namedLocation, decimalLatitude,
  ...
  data_fish = dplyr::left_join(data_fish, fsh_taxa, by = c("taxonID", "scientificName"))
  data_fish = dplyr::select(data_fish, -year, -month) %>% dplyr::distinct()
  return(data_fish)
}
